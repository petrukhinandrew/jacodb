...
class InMemoryIlHierarchyAnnotatedTest {
    companion object {
    ...
    fun testGenericChildren() {
        ...
        val response1 = runBlocking { InMemoryIlHierarchy.query(publication, intRequest).toList() }
        response1.forEach {
            assertTrue(it.isGenericType && !it.isGenericDefinition)
            assertEquals(publication.int32(), it.genericArgs.first())
        }
        assertEquals(setOf("SingleParamStruct`1", "SingleParamAny`1"), response1.map { it.name }.toSet())
        ...
        val response2 = runBlocking { InMemoryIlHierarchy.query(publication, strRequest).toList() }
        ...
        assertEquals(setOf("SingleParamClass`1", "SingleParamAny`1"), response2.map { it.name }.toSet())
    ...
    fun testDefaultCtorSingleArg() {
        ...
        assertEquals(setOf("DefaultCtorClass`1"), response1.map { it.name }.toSet())
        ...
        testEntries.forEach { entry ->
            ...
            val expectationRefs: List<IlTypeRef> =
                ((entry.attributes.first {
                    it.type.fullname == testEntryAttrType
                } as IlAttributeImpl).constructorArgs[0] as IlArrayConstant).values as List<IlTypeRef>
            val expectation = expectationRefs.map { it.referencedType }.sortedBy { it.typeToken }

            val actual = runBlocking { InMemoryIlHierarchy.query(publication, req) }
